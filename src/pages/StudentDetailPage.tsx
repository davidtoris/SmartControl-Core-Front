...
import { 
  ...
  Clock, Award, Star, Plus, AlertCircle
} from 'lucide-react';
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: '24px' }}>
                  ...
                  <div className="bento-card" style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                    ...
                    <div style={{
                      ...
                      if (!selectedStudent.fotoEvidencia) {
                        const name = prompt('Ingrese el nombre de la foto de recibido (ej. foto_recibido.jpg):', `evidencia_${selectedStudent.name.toLowerCase().replace(/\s+/g, '_')}_recibido.jpg`);
                        if (name) {
                          await useAppStore.getState().updateStudentTracking(selectedStudent.id, { 
                            fotoEvidencia: name
                          });
                ...
                <div className="bento-card">
                  ...
                  <div style={{ padding: '16px', background: 'rgba(239, 68, 68, 0.05)', border: '1px solid rgba(239, 68, 68, 0.2)', borderRadius: '12px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <button 
                      ...
                      onClick={async () => {
                        const payAmt = prompt(`Ingrese el monto a pagar para ${selectedStudent.name}:`, "1000");
                        if (payAmt && parseFloat(payAmt) > 0) {
                          try {
                            const receiptUrl = await useAppStore.getState().recordStudentPayment(selectedStudent.id, parseFloat(payAmt));
                            if (receiptUrl) {
                              const downloadConfirm = confirm('¡Abono registrado con éxito!\n¿Desea descargar el comprobante de pago oficial PDF ahora?');
                              if (downloadConfirm) {
